...
- TypeScript in `src/Gobchat.App/resources/ui` is compiled by `Microsoft.TypeScript.MSBuild` as part of the Gobchat.App build (config: [tsconfig.json](src/Gobchat.App/resources/ui/tsconfig.json), target ES2021, strict). Emitted `.js` files sit next to their `.ts` sources.
- A unit test suite covers the unit-testable logic (see **Testing** below); CI runs the build + unit suites on every PR and `master` push, and broader verification is still manual (run `GobchatEx.exe`, which requires FFXIV for memory features). The overlay renders through the OS WebView2 Evergreen runtime (the build only ships the small `Microsoft.Web.WebView2.*` wrappers + `WebView2Loader.dll`); nothing is downloaded on first start.

## Testing

Two suites live under `tests/` (the C# ones are in `Gobchat.sln`). They are deterministic (no FFXIV/desktop dependency) and run in CI on every PR and `master` push (`.github/workflows/ci.yml`).
...
- **Gobchat.LogConverter** (`src/Gobchat.LogConverter`) — a standalone WinForms tool that converts written chat logs. (Auto-update is handled in-app by `AppModuleUpdater` via Velopack's `UpdateManager`; the old standalone `GobUpdater` helper was removed in the Velopack migration.)

## Architecture

### Module system (C# side)

Startup is a linear module pipeline: `MainEntry` → [GobchatApplicationContext.cs](src/Gobchat.App/Core/Runtime/GobchatApplicationContext.cs), which initializes an ordered list of `IApplicationModule`s and disposes them in reverse order on shutdown. Modules live in `src/Gobchat.App/Module/*`; shared/core code (chat message model, config manager, runtime, util) lives in `src/Gobchat.App/Core/*`.
...
- C#→JS: events are serialized to JSON and dispatched as DOM custom events (`ChatMessagesWebEvent` etc., see `Module/UI/WebEvents` and `src/Gobchat.WebRenderer/Web/JavascriptEvents`) through `CoreWebView2.ExecuteScriptAsync`.

### Web UI (TypeScript)

Lives in `src/Gobchat.App/resources/ui`. Entry points: `gobchat.html`/`gobchat.ts` (the overlay) and `config/` (the settings dialog). Shared code is in `modules/` (`Chat.ts`, `Config.ts`, `Databinding.ts`, `Style.ts`, `Locale.ts`, …); `gobchat/*.js` is the older pre-TypeScript layer. Globals (`gobConfig`, `gobLocale`, `gobStyles`, `gobChatManager`) are declared in `globals.d.ts` and set up in `gobchat.ts`. Themes are CSS in `resources/ui/styles`.
